...
impl TaskCreateFeature {
    ...
    async fn execute<'p>(
    ...
    ) -> Result<crate::models::Task, crate::errors::feature::TaskCreateError> {
        use crate::errors::{
            feature::TaskCreateError as Error,
        ...
        let result =
            crate::repositories::TaskRepository::create(&mut *database_connection, params.task)
                .await;

        match result {
            Ok(task) => Ok(task),
            Err(err) => match err {
                crate::errors::repository::TaskCreateError::ProjectNotFound => {
    ...
    async fn controller(
        ...
        use crate::errors::feature::TaskCreateError as Error;
